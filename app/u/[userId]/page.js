 ...
 * Shows name, avatar (with Persian placeholder if null), and public wishlists.
...
export default async function PublicProfilePage({ params }) {
  ...
  const { name, image, wishlists } = profile
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-background px-4 py-10">
      <div className="mx-auto max-w-3xl">
        ...
        {/* Wishlists grid */}
        {wishlists.length === 0 ? (
          <p className="text-center text-muted-foreground">هنوز هیچ آرزوی عمومی ثبت نشده است.</p>
        ) : (
          <>
            <h2 className="mb-4 text-lg font-semibold text-foreground">آرزوها</h2>
            <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {wishlists.map((wishlist) => (
                <li key={wishlist.id}>
                  ...
                  >
                    <h3 className="font-semibold text-foreground">{wishlist.title}</h3>
