...
const SHIPPING_ADDRESS_MAX = 500
const TRACKING_CODE_MAX = 100

/**
 * Validates purchase-proof fields a reserver can attach to their
 * reservation after buying the gift: a receipt image URL, a shipping
 * address, and/or a tracking code. All fields are optional and independent.
 * @param {{ receiptImageUrl?: string, shippingAddress?: string, trackingCode?: string }} data
 * @returns {{ valid: boolean, error?: string, field?: string }}
 */
export function validateReservationProof(data) {
  const { receiptImageUrl, shippingAddress, trackingCode } = data ?? {}

  if (receiptImageUrl !== undefined && receiptImageUrl !== null && receiptImageUrl !== '') {
    if (!validateUrl(receiptImageUrl).valid) {
      return {
        valid: false,
        error: 'آدرس تصویر رسید باید با http:// یا https:// شروع شود.',
  ...
  if (shippingAddress !== undefined && shippingAddress !== null && shippingAddress !== '') {
    if (typeof shippingAddress !== 'string' || shippingAddress.length > SHIPPING_ADDRESS_MAX) {
      return {
        valid: false,
        error: `آدرس ارسال نباید بیشتر از ${SHIPPING_ADDRESS_MAX} کاراکتر باشد.`,
  ...
  if (trackingCode !== undefined && trackingCode !== null && trackingCode !== '') {
    if (typeof trackingCode !== 'string' || trackingCode.length > TRACKING_CODE_MAX) {
      return {
        valid: false,
        error: `کد رهگیری نباید بیشتر از ${TRACKING_CODE_MAX} کاراکتر باشد.`,
        field: 'trackingCode',
      }
    }
  }
